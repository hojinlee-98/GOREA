...
gorea_enviromnet <- function(directory = NULL) {
  ...
  GO.LARGE <<- readRDS(paste0(directory, "/GeneOntology/v7.5.1.Mm/GOBP/20250206_msigDB_GO_ancestor_large_hj.rds"))
...
gorea_getHeatmap <- function(k_val = NULL,
  ...
  if (top_ancestor_annotation == T) {
    ...
    set.seed(1234); w_large <- GOSemSim::mgoSim(unique(res$ANCESTOR_GOID), unique(res$ANCESTOR_GOID), semData = godata_GO, measure = "Rel", combine = NULL)
    set.seed(1234); na_goid <- names(which(is.na(w_large[1,]))) # select NA goid.
    goids_large <- unique(res$ANCESTOR_GOID)[which(!unique(res$ANCESTOR_GOID) %in% na_goid)]
    set.seed(1234); w_large <- GOSemSim::mgoSim(goids_large, goids_large, semData = godata_GO, measure = "Rel", combine = NULL) # w is similarity
    set.seed(1234); dmat <- stats::as.dist(1 - w_large) # distance matrix.
    set.seed(1234); hc <- stats::hclust(dmat, method = "ward.D") # h.clust using ward.D.
    set.seed(1234); large_hclust_wardD <- stats::cutree(hc, k = top_ancestor)
    large_hc_res_df <- data.frame(ANCESTOR_GOID = names(large_hclust_wardD),
                                  cluster = large_hclust_wardD,
                                  row.names = NULL)
    
    large_hc_res_df <- merge(large_hc_res_df, res, by = "ANCESTOR_GOID", all.x = T)
    res_filtered <- large_hc_res_df %>% dplyr::group_by(cluster) %>% dplyr::arrange(desc(n)) %>% dplyr::group_by(cluster) %>% dplyr::slice(1)
    ...
    for (i in 1:nrow(res_filtered)) {
      ...
      df1 <- df1 %>% dplyr::mutate(!!sym(mycolname) :=  case_when(GOID %in% child_terms ~ "child term",
                                                                  T ~ " "))
      n_child <- df1 %>% dplyr::filter(!!sym(mycolname) == "child term") %>% nrow()
    ...
    for (i in 1:top_ancestor) { ht1@anno_list[[names(ht1@anno_list)[i]]]@color_mapping@levels <- c("child term", " ")}
