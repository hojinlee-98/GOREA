...
gorea_enviromnet <- function(directory = NULL) {
  ...
  GO.LARGE <<- readRDS(paste0(directory, "/GeneOntology/v2024.1.Hs/GOBP/20250206_msigDB_GO_ancestor_large_hj.rds"))
  
  if (!("godata_GO" %in% ls(envir=.GlobalEnv))) {
    set.seed(1234); godata_GO <<- godata('org.Hs.eg.db', ont="BP", computeIC = T) # make IC score using GOsemsim package 
  }
}
  ...
  if (ancestor_annotation == T) {
    ...
    p1 <- Heatmap(mat = w, col = col_fun, name = "Similarity",
                  ...
                  right_annotation = rowAnnotation(ggplot1 = anno_empty(height = unit(15, "cm"), width = unit(0.7, "cm")),
                                                   show_annotation_name = FALSE, gp = gpar(col = "white", lwd = 2),
                                                   textbox = anno_textbox(textbox_list, anno_list, add_new_line = TRUE, word_wrap = TRUE, max_width = unit(10, "cm")),
                                                   gap = unit(1, "mm"))
    )
