...
gorea_enviromnet("/Users/hojin/Dropbox/project/GOREA/20250401/")
...
#msigDB_GO_ancestor_large <- msigDB_GO_ancestor_large %>% dplyr::filter(!ANCESTOR_GOTERM %in% c("multicellular organismal process", "organic substance metabolic process", "cellular metabolic process", "primary metabolic process", "nitrogen compound metabolic process"))

saveRDS(msigDB_GO_ancestor_large, "/Users/hojin/Dropbox/project/GOREA/20250401/GeneOntology/v2024.1.Hs/GOBP/20250206_msigDB_GO_ancestor_large_hj.rds")
...
#msigDB_GO_ancestor_large <- msigDB_GO_ancestor_large %>% dplyr::filter(!ANCESTOR_LEVEL %in% c("organic substance metabolic process", "cellular metabolic process", "nitrogen compound metabolic process"))

saveRDS(msigDB_GO_ancestor_large, "/Users/hojin/Dropbox/project/GOREA/20250401/GeneOntology/v2024.1.Mm/GOBP/20250206_msigDB_GO_ancestor_large_hj.rds")
...
gorea_enviromnet <- function(directory = NULL) {
  ...
  GOGMT <- readRDS(paste0(directory, "/GeneOntology/v7.5.1.Mm/GOBP/gobp_7.5.1.rds"))
  GOID_TERM <- read.table(paste0(directory, "/GeneOntology/v7.5.1.Mm/GOBP/gobp_id_term_gene_sets.txt"),
                          header = T,
                          sep = "\t") # this file is made from json file
  GO.ANCESTOR <<- readRDS(paste0(directory, "/GeneOntology/v7.5.1.Mm/GOBP/20250124_m5.go.bp.v7.5.1.Mm_ANCESTOR_df_hj.rds"))
  GO.LEVEL <<- readRDS(paste0(directory, "/GeneOntology/v7.5.1.Mm/GOBP/20250124_m5.go.bp.v7.5.1.Mm_LEVEL_df_hj.rds"))
...
for (i in 1:112) { # Only 1~112 values can be tested, because the test allow at least 7 values and different value for each other.
  test_values <- test_values[-1]
  go_val_tmp <- test_values[1]
  res_tmp <- grubbs.test(test_values, type = 10) 
  pval_tmp <- res_tmp$p.value
...
# histogram
level1_2_count_df <- level1_2_count_df %>% dplyr::mutate(Outlier = case_when(n %in% c(4861, 3313, 3057) ~ T,
                                                                             T ~ F))
pdf("/Users/hojin/Dropbox/project/GOREA/20250401/GeneOntology/v7.5.1.Mm/GOBP/20250206_ancestor_outlier_grubbs_test_hist_mouse_v7.5.1_hj.pdf", width = 4.5, height = 4.5)
level1_2_count_df%>% 
  ggplot(aes(x = n)) + 
  xlab("Number of times considered an ancestor") +
  ylab("Count") +
...
saveRDS(msigDB_GO_ancestor_large, "/Users/hojin/Dropbox/project/GOREA/20250401/GeneOntology/v7.5.1.Mm/GOBP/20250206_msigDB_GO_ancestor_large_hj.rds")
